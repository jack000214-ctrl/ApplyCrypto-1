...
class AnyframeContextGenerator(BaseContextGenerator):
    ...
    USE_TOKEN_LIMIT = False  # Set to False to disable token limit
        ...
        try:
            ...
            for imp in biz_imports:
                if self._match_import_to_file_path(imp, target_dqm_files):
                    logger.debug(f"      ✓ Found target DQM in chain: {Path(biz_file).name}")
                    return current_chain
                    
                if self._match_import_to_file_path(imp, target_dem_files):
                    logger.debug(f"      ✓ Found target DEM in chain: {Path(biz_file).name}")
                    return current_chain
                    
            # Same-package에서도 DQM/DEM 확인
            same_package_dqm = self._find_same_package_references(biz_file, target_dqm_files)
            if same_package_dqm:
                logger.debug(f"      ✓ Found target DQM (same-package) in chain: {Path(biz_file).name}")
                return current_chain
                
            same_package_dem = self._find_same_package_references(biz_file, target_dem_files)
            if same_package_dem:
                logger.debug(f"      ✓ Found target DEM (same-package) in chain: {Path(biz_file).name}")
                return current_chain
            ...
            for imp in biz_imports:
                ...
                if nested_biz:
                    result = self._find_biz_chain_to_target(
                        nested_biz, base_src_path, biz_files,
                        target_dqm_files, target_dem_files,
                        java_parser, visited, current_chain
                    )
                    if result:
                        return result # 타겟 찾음!
            ...
            for imp in biz_imports:
                ...
                if nested_biz_path and nested_biz_path not in visited:
                    result = self._find_biz_chain_to_target(
                        nested_biz_path, base_src_path, biz_files,
                        target_dqm_files, target_dem_files,
                        java_parser, visited, current_chain
                    )
                    if result:
    ...
    def _match_import_to_file_path(
        ...
        for file_path in target_files:
            ...
            if match_found:
                logger.debug(f"      ✓ Matched: {import_statement} -> {file_path_obj.name}")
                return file_path
        return None
    ...
    def generate(
        ...
        repository_files = layer_files.get("repository", []) # VO/SVO/DVO 파일들
        dem_daq_files = layer_files.get("dem_daq", []) # DQM/DEM 파일들
        
        # SVC 파일에서 실제 Service 구현체만 필터링 (VO 파일 제외)
        svc_files_all = [
            x for x in svc_files_raw
            if not (x.endswith("VO.java") or x.endswith("SVO.java") or x.endswith("DVO.java"))
        ]
        ...
        # 패턴: IAOfrCaseInfoMngSVCImpl.java -> IAOfrCaseInfoMngSVC.java
        ...
        for impl_file in svc_impl_files:
            ...
            interface_name_candidate2 = "I" + impl_path.stem.replace("Impl", "") + ".java"
            
            # 같은 디렉토리 또는 상위 디렉토리에서 Interface 찾기
            possible_interface_path1 = impl_path.parent.parent / interface_name_candidate1
            possible_interface_path2 = impl_path.parent.parent / interface_name_candidate2
            
            if possible_interface_path1.exists():
                impl_to_interface[impl_file] = str(possible_interface_path1)
                logger.debug(f"      Matched interface: {impl_path.name} -> {possible_interface_path1.name}")
            elif possible_interface_path2.exists():
                impl_to_interface[impl_file] = str(possible_interface_path2)
                logger.debug(f"      Matched interface: {impl_path.name} -> {possible_interface_path2.name}")
        ...
        for svc_impl_file in anchor_files:
            ...
            try:
                ...
                for import_stmt in impl_imports:
                    ...
                    if matched and matched not in matched_biz_files:
                        ...
                        if chain:
                            ...
                            matched_biz_files.extend([b for b in chain if b not in matched_biz_files])
                
                # Step 2: layer_files에 없는 BIZ도 확인 (fallback)
                if base_src_path:
                    for import_stmt in impl_imports:
                        if "biz" not in import_stmt.lower() and "BIZ" not in import_stmt:
                            continue
                        ...
                        # Import -> 파일 경로 변환
                        biz_file_path = self._convert_import_to_file_path(import_stmt, base_src_path)
                ...
                fallback_biz_count = sum(
                    1 for chain in all_biz_chains
                    for biz in chain
                    if not self._match_import_to_file_path(
                        Path(biz).stem, biz_files
                    )
                )
                ...
                for dqm_file in matched_dqm_files:
                    try:
                        ...
                        if not dqm_error:
                            dqm_classes = java_parser.extract_class_info(dqm_tree, Path(dqm_file))
                            if dqm_classes:
                                dqm_class = next(
                                    (c for c in dqm_classes if c.access_modifier == "public"),
                                    dqm_classes[0],
                                )
                ...
                for dem_file in matched_dem_files:
                    try:
                        ...
                        if not dem_error:
                            dem_classes = java_parser.extract_class_info(dem_tree, Path(dem_file))
                            if dem_classes:
                                dem_class = next(
                                    (c for c in dem_classes if c.access_modifier == "public"),
                                    dem_classes[0],
                                )
