...
### v2.6+ Enum 타입 및 AST 파싱 강화 (2026.03.16)
- **Enum 클래스 지원** ✅: enum_declaration 인식, Enum 필드/메서드도 엔드포인트 추출 대상
- **경로 기반 순환 참조 방지** ✅: AST 파싱 안정성 향상 (복잡한 Java 구조도 정확히 처리)
- **메서드 경계 인식 강화** ✅: 중첩된 내부 클래스/Enum도 정확히 구분
