...
본 명세서는 **두 가지 수정 방식**을 지원합니다. `config.json`의 `modification_type` 필드로 구분됩니다:

| 수정 방식 | 적용 위치 | 구현 방식 | 수정 시점 | Pipeline | modification_type |
|---|---|---|---|---|---|
| **TypeHandler 방식** | mapper.xml TypeHandler | SQL 기반 분석 | 수정 **후** | 9 단계 | 소문자로 "typehandler" 포함 |
| **ThreeStep 방식** | SVC/SVCImpl/BIZ 소스 코드 | 코드 기반 분석 (Call Graph 기반) | 수정 **전** (diff 이용) | 9 단계 | "typehandler" 미포함 (예: ThreeStep, TwoStep) |
...
## 2단계. 쿼리 분석 (TypeHandler 방식의 경우)

**정의**: APPLYCRYPTO modify 수행 후 암복호화 필드 매핑  
**출력**: `{target_project}\.applycrypto\modify_results\{timestamp}\{table_name}\step1_query_analysis.json`
...
- ✅ 검증 강화: 4가지 항목 검증 (modification_type, ksignutil_methods 존재, 메서드 형식, 필수 필드)
...
**Step 4**: 런타임 호출빈도 데이터 로드 및 메서드 매핑 (ThreeStep 방식)
...
**Step 5**: call_graph.json과 crypto_weight 매핑 (TypeHandler 방식 호환)
...
1. **Config 파일 로드 기능**
   - `modification_type` 기반 자동 감지
   - `ksignUtils` 배열 읽기

2. **Pipeline Dispatcher**
   - TypeHandler 방식 vs ThreeStep 방식 자동 분기

3. **TypeHandler 방식 파이프라인 (mapper.xml 기반)**
   - 모든 9개 단계 구현
   - 호환성 100% 유지

4. **ThreeStep 방식 파이프라인 (LLM 분석 기반)**
   - 구조: 완료
   - 가중치 계산: 완료
   - 헬퍼 메서드: 완료
...
6. **Step 1 개선사항 구현** ✅ (2026.03.03)
   ...
   - ✅ **개선안 2**: 검증 로직 강화
     ...
     * 4가지 검증: modification_type, ksignutil_methods, 메서드 형식, 필수 필드
   - ✅ **개선안 3**: 에러 메시지 개선
     * 파일 경로, 원인, 해결책 포함
     * 구체적 예제 코드 제시
   - ✅ **개선안 1**: Step 1 로직 명확화
...
**필수 필드**:
- `modification_type`: "ThreeStep" (또는 "TwoStep") - Step 1에서 검증
...
```bash
# TypeHandler 방식 (mapper.xml 기반)
python main.py generate-ksign-report --config config.json

# ThreeStep 방식 (LLM 분석 기반)
python main.py generate-ksign-report --config config_threestep.json
```
...
**Step 1 성공 시**:
```
[Framework] ThreeStep 방식 - SVC/SVCImpl/BIZ 코드 기반 처리
